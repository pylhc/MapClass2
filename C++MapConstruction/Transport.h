...
template <class T> Polynom<T>  Q11(double L, double K1L, int order, Polynom<T> d) {
	Polynom<T> K = (d + 1).pinv() * abs(K1L/L); 
	return cosp(sqrtp(K) * L);
} 

template <class T> Polynom<T> Q12(double L,double K1L, int order, Polynom<T> d) {
	Polynom<T> K =  (d + 1).pinv() * abs(K1L/L);
 	return (sqrtp(K).pinv() * sinp(sqrtp(K) * L))/(d + 1);
}

template <class T> Polynom<T> Q21(double L,double K1L, int order, Polynom<T> d) {
	Polynom<T> K =  (d + 1).pinv() * abs(K1L/L);
 	return sqrtp(K) * sinp(sqrtp(K) * L) * (d + 1) * (-1);
}

template <class T> Polynom<T> Q33(double L,double K1L, int order, Polynom<T> d) {
	Polynom<T> K =  (d + 1).pinv() * abs(K1L/L);
 	return coshp(sqrtp(K) * L);
}

template <class T> Polynom<T> Q34(double L,double K1L, int order, Polynom<T> d) {
	Polynom<T> K =  (d + 1).pinv() * abs(K1L/L);
 	return sqrtp(K).pinv() * sinhp(sqrtp(K) * L)/(d + 1);
}

template <class T> Polynom<T> Q43(double L,double K1L, int order, Polynom<T> d) {
	Polynom<T> K =  (d + 1).pinv() * abs(K1L/L);
 	return sqrtp(K)*sinhp(sqrtp(K) * L) * (d + 1);
}

template <class T> Polmap<T> QFMap(double L,double K1L, int order, Polynom<T> x, Polynom<T> px, Polynom<T> y, Polynom<T> py, Polynom<T> d, Polynom<T> s) {
    unordered_map<string, Polynom<T>> mp;
  	Polynom<T> q11 = Q11(L, K1L, order, d);
  	Polynom<T> q12 = Q12(L, K1L, order, d);
  	Polynom<T> q21 = Q21(L, K1L, order, d);
  	Polynom<T> q33 = Q33(L, K1L, order, d);
  	Polynom<T> q34 = Q34(L, K1L, order, d);
  	Polynom<T> q43 = Q43(L, K1L, order, d);
...
template <class T> Polmap<T> QDMap(double L,double K1L, int order, Polynom<T> x, Polynom<T> px, Polynom<T> y, Polynom<T> py, Polynom<T> d, Polynom<T> s) {
  	unordered_map<string, Polynom<T>> mp;
  	Polynom<T> q11 = Q11(L, K1L, order, d);
  	Polynom<T> q12 = Q12(L, K1L, order, d);
  	Polynom<T> q21 = Q21(L, K1L, order, d);
...
template <class T> Polmap<T> mapForElement(unordered_map<string, any> e, int order, vector<vector<Polynom<T>>> v,
	...
	string octupole = "OCTUPOLE";
		...
		if (l != 0) { 
        		if (k1l > 0)
          		return QFMap(l, k1l, order, x, px, y, py, d, s);
        		else
          		return QDMap(l, k1l, order, x, px, y, py, d, s);
          }
