...
static vector<Polynom<complex<double>>> EQ(int n, int order) {
	...
	lst.push_back(init);
	for (int i = 2; i <= n; i ++) {
		lst.push_back((lst[i - 2] * init) * (1./(i)));;// * (1./factorial(i)));
	}
	return lst;
}
...
//	cout << keyword << endl;
	int order = x.order;
	double eps = x.eps;
	Polynom<T> l = Polynom<T>(order, eps, "l" + lexical_cast<string>(index), atof(e["L"].c_str()));
	Polynom<T> k1l = Polynom<T>(order, eps, "k1l" + lexical_cast<string>(index), atof(e["K1L"].c_str()));
	Polynom<T> k2l = Polynom<T>(order, eps, "k2l" + lexical_cast<string>(index), atof(e["K2L"].c_str()));
	Polynom<T> k3l = Polynom<T>(order, eps, "k3l" + lexical_cast<string>(index), atof(e["K3L"].c_str()));
	Polynom<T> k4l = Polynom<T>(order, eps, "k4l" + lexical_cast<string>(index), atof(e["K4L"].c_str()));
	Polynom<T> angle = Polynom<T>(order, eps, "angle" + lexical_cast<string>(index), atof(e["ANGLE"].c_str()));
	Polynom<T> delta = Polynom<T>(order, eps, "deltap"+ lexical_cast<string>(index), atof(e["DELTAP"].c_str()));
	string drift = "DRIFT";
