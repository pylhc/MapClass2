...
)
{
 ...
 //   assert((arrayLength >= MIN_SHORT_ARRAY_SIZE) && (arrayLength <= MAX_SHORT_ARRAY_SIZE));
    ...
    int dimGrid = arrayLength / (4 * THREADBLOCK_SIZE);
    if ( dimGrid == 0 )
	dimGrid = 1;
	  

    scanExclusiveShared<<<dimGrid, THREADBLOCK_SIZE, 0, stream>>>(
        (uint4 *)d_Dst,
        (uint4 *)d_Src,
        arrayLength
    );
...
)
{
  ...
  //  assert((arrayLength >= MIN_LARGE_ARRAY_SIZE) && (arrayLength <= MAX_LARGE_ARRAY_SIZE));
    ...
    scanExclusiveShared<<<(arrayLength) / (4 * THREADBLOCK_SIZE), THREADBLOCK_SIZE, 0, stream>>>(
        (uint4 *)d_Dst,
        (uint4 *)d_Src,
        4 * THREADBLOCK_SIZE
    );
    getLastCudaError("scanExclusiveShared() execution FAILED\n");
    uint *d_Buf;
    checkCudaErrors(cudaMalloc((void **)&d_Buf, (arrayLength / (4 * THREADBLOCK_SIZE)) * sizeof(uint)));
    ...
    uniformUpdate<<<arrayLength / (4 * THREADBLOCK_SIZE), THREADBLOCK_SIZE, 0, stream>>>(
        (uint4 *)d_Dst,
        (uint *)d_Buf
    );
