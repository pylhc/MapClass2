  ...
  double inits = omp_get_wtime();
	...
	for (int i = 0; i < nr_terms; i ++){
		if (!terms[i].empty()) {
		    ...
		    int x1 = (dimAux % threads.x == 0) ? 0 : 1;
		    int y1 = (dimInput % threads.y == 0) ? 0 : 1;
		    dim3 grid(dimAux/BL_SIZE_X + x1, dimInput/BL_SIZE_Y + y1);

		    if (terms[i].size() == 1){ 
			      multiply_pols_truncate_key<BL_SIZE_X, BL_SIZE_Y, NRVARS, MAX_E> <<< grid, threads, 0, streams[i] >>>
				    (p_result[i].first, e_keys + i * result_size,
				    p_aux[i].first, e_input + index_input_exp,
				    p_result[i].second, p_aux[i].second, c_input + index_input_coeff, 
				    dimResult, dimAux, dimInput, order, stencilp[i], input_coeff[i]); 
		    } 
		    else {	
			     multiply_pols_truncate<BL_SIZE_X, BL_SIZE_Y, NRVARS, MAX_E> <<< grid, threads, 0, streams[i] >>>
			    	(p_result[i].first, p_aux[i].first, e_input + index_input_exp,
			    	p_result[i].second, p_aux[i].second, c_input + index_input_coeff, 
			     	dimResult, dimAux, dimInput, order, stencilp[i]); 	 			    
		    }
    		}
    	}
		...
		if (!terms[i].empty()) {	
				...
				scanExclusiveShort(indicesp[i], stencilp[i], stencil_size, streams[i]);
			else  scanExclusiveLarge(indicesp[i], stencilp[i], stencil_size, streams[i]);
			checkCuda(cudaMemcpyAsync(&sizes_aux[i], indicesp[i] + dimRes[i], sizeof(uint), cudaMemcpyDeviceToHost, streams[i]));
		}
	}
	cudaThreadSynchronize();
	for (int i = 0; i < nr_terms; i ++){
		if (!terms[i].empty()) {	
			...
			if (terms[i].size() == 1){ 
        			stream_compactation_key<NRVARS><<<dimGrid, THREADBLOCK_SIZE, 0, streams[i]>>>(e_keys + i * result_size, p_result[i].second, stencilp[i], indicesp[i], dimRes[i], sizes_aux[i], keys + i * result_size, p_aux[i].second); 
			}
			else {
        			 stream_compactation<THREADBLOCK_SIZE, NRVARS><<<dimGrid, THREADBLOCK_SIZE, 0, streams[i]>>>(p_result[i].first, p_result[i].second, stencilp[i], indicesp[i], dimRes[i], sizes_aux[i], p_aux[i].first, p_aux[i].second);
    			}
			terms[i].pop_front();
			//#pragma omp critical
			iterations --;
