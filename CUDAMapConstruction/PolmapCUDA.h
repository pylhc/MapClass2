...
int multPol(int* exp_A, double* coeff_A, int* exp_B, double* coeff_B, int* exp_C, double* coeff_C,
             int &dimA, int &dimB, int &order, int &nvars);
...
static Polmap<double> compose(Polmap<double> first, Polmap<double> other,
                        int* exp_C, int* exp_A, int* exp_B,
                        double* coeff_C, double* coeff_A, double* coeff_B) {
        unordered_map<string, Polynom<double>> result_pols;
        unordered_map<string, Polynom<double>> temp;
        ...
        for (typename unordered_map<string, Polynom<double>>::const_iterator itmap = first.pols.begin(); itmap != first.pols.end(); ++itmap) {
                ...
                Polynom<double> newpol = Polynom<double>(p.order, p.eps, p.vars, 0);
                vector<int> zero(p.vars.size(), 0);
                for (typename unordered_map<vector<int>, double, container_hash<vector<int>>>::iterator i = p.terms.begin(); i != p.terms.end(); ++i) {
                        m.clear();
                        Polynom<double> coef = Polynom<double>(p.order, p.eps, p.vars, i->second);
                        for (int k = 0; k < p.vars.size(); k ++) {
                                typename unordered_map<string, Polynom<double>>::const_iterator found = other.pols.find (p.vars[k]);
                                if (found == other.pols.end())
                                        coef = coef * Polynom<double> (p.order, p.eps, p.vars[k], i->first[k]);
                                else
                                        for (int a = 0; a < i->first[k]; a ++)
                                                m.push_back(p.vars[k]);
                        }
                        if (m.size() > 0) {
                                while (m.size() > 1) {
                                        ...
                                        m.pop_back();
 					string pp = first + " " + second;
                                        typename unordered_map<string, Polynom<double>>::const_iterator found = temp.find (pp);
                                        if (found == temp.end()) {
                                            int dimA = temp[first].terms.size();
                                            int dimB = temp[second].terms.size();

                                            if (dimA >= POL_SIZE && dimB >= POL_SIZE) {
                                                int nvars = p.vars.size();
                                                temp[first].getPolRaw((int*)exp_A, coeff_A);
                                                temp[second].getPolRaw((int*)exp_B, coeff_B);
                                                int nb_terms = multPol(exp_A, coeff_A, exp_B, coeff_B, exp_C, coeff_C, dimA, dimB, p.order, nvars);
                                                Polynom<double> newv = Polynom<double>(p.order, p.eps, p.vars, exp_C, coeff_C, nb_terms);
                                                temp.insert(make_pair(pp, newv));
                                            }
                                            else {
                                                Polynom<double> newv = temp[first] * temp[second];
                                                temp.insert(make_pair(pp, newv));
                                            }
 					}
                                        m.push_back(pp);
